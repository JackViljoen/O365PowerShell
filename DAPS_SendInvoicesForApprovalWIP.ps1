...
function get-wfHistoryAsHTML() {
    ...
    if ($SPHistory) {
        ...
        foreach ($SPH in $SPHistory) {
            $Icon = ""
            if($SPH.Attachments) {
                $Icon = "<div class='attachmment' data-item='$($SPH.ID)'>XXX$($SPH.ID)XXX</div>"
                $attachments+= $SPH.ID
            }
            if ($SPH.OData__wfAction.indexOf("far fa-save fa-fw fa-2x iBlue") -eq -1 -or $null -ne $SPH.OData__wfLogComment -or $SPH.Attachments) {
                $HistoryTable += "<div class='hitem'><div class='hitemheader'>$(get-date($SPH.OData__wfTime) -format "dd MMM HH:mm") $($SPH.OData__wfUser) : $($SPH.OData__wfAction.replace('fa-2x','')) </div><div class='hcomment'>$($SPH.OData__wfLogComment) </div>$($Icon)</div>"
            }
        }
    }
...
$Sites = @(
    @{"Active" = $true;  "SiteUrl"  = "https://pacificlife.sharepoint.com/sites/PLRe-REDAPS";  "BU" = "RE";  "From" = "accounts@underwriteme.co.uk" };
    @{"Active" = $false;  "SiteUrl"  = "https://pacificlife.sharepoint.com/sites/PLRe-DCDAPS";  "BU" = "DC";  "From" = "accountspayableUK@pacificlifere.com" };
    @{"Active" = $false;  "SiteUrl"  = "https://pacificlife.sharepoint.com/sites/PLRe-AUDAPS";  "BU" = "AU";  "From" = "Accounts.PayableAUS@PacificLifere.com" };
)

#Lets reach out to the destination site and get the connection and the roles
$destConnection = Connect-PnPOnline -Url "https://pacificlife.sharepoint.com/sites/PLRe-tDAPSApprovalsUAT" -Credentials $credential -ReturnConnection 
...
forEach ($S in $sites) {
    If ($S.Active) {
            ...
            if ($SPInvoice.Id -ge 210) {
                ...
                if($HistoryHTMLObj.attach){
                    foreach($AT in $HistoryHTMLObj.attach){
                        # get the attachmwent nasd asend over 
                        write-host $AT
                        ...
                        foreach($AF in $AttachmentFiles){  
                            Add-PnPFile -Connection $destConnection -Path $AF.FullName -Folder "Attachments" -Values @{BusRef=$AT} 
                            start-sleep 1
                            remove-Item $AF.FullName
                        }
                        #now replace the holder with a HREF 
                        $HistoryHTML = $HistoryHTML.Replace("XXX$($AT)XXX", "<a href='/sites/PLRe-tDAPSApprovalsUAT/Attachments/$($AF.Name)' target='_blank'> X </a>")
                    }
                }

                write-host $HistoryHTML -ForegroundColor Yellow
                <#
        ...
        <#
        #NEW BIT (25th APRIL) TRASH THE CANCELLED ONES 
