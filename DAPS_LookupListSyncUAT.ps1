...
$credential = New-Object System.Management.Automation.PsCredential($logaccountName, $logencrypted)
...
#& where is it going too?  UAT UAT UAT 
$FollowerLists = "sites/PLRe-tdapsApprovalsUAT/Lists/Apportionments"

$CopyData = $true ## lets assume we will copy stuff out 

foreach ($SourceSiteUrl in $Sites){
    $sourceConnection = Connect-PnPOnline -Url $sourceSiteURL -ReturnConnection -Credentials $credential -ErrorAction SilentlyContinue -ErrorVariable ErrVar
    ...
    if ($Errvar[0].Count -eq 0) {
       ...
       if ($Errvar[0].Count -eq 0) {
            ...
            if ($ListLastUpdate -gt $LastRun) {
                ...
                #finally get the items from the source list i think i need them all to start with as i need to be able to identify the deletes
                $sourceItems = Get-PnPlistItem -Connection $sourceConnection -List  $sourceListName  -PageSize 500 ### | Where-Object {$_.FieldValues.APPCategory -eq "Additional Cost Centre"}
                        ...
                        if ($null -ne $l) {
                            ...
                            if($InvalidLookupFollower){
                                ...
                                foreach($ILF in $InvalidLookupFollower){
                                    $deadThing = Remove-PnPListItem -Connection $UpdateConnection -List $p.LibPath -Identity $ILF.Id -Force # -Batch $DeleteTransactions
                                }
                                    ...
                                    "=>" {
                                            ...
                                            if($DR.Count -eq  1){ 
                                                if ($null -ne $DR) {
                                                    ...
                                                    $a = Remove-PnPListItem -Connection $UpdateConnection -List $p.LibPath -Identity $DR.Id -Force -ErrorAction SilentlyContinue -ErrorVariable ErrVar
                                                    if ($Errvar[0].Count -gt 0) {
                                                ...
                                                for($UI=0; $UI -lt $DR.Count; $UI++){ 
                                                    $a = Remove-PnPListItem -Connection $UpdateConnection -List $p.LibPath -Identity $DR[$UI].Id -Force -ErrorAction SilentlyContinue -ErrorVariable ErrVar
                                    ...
                                    if($DD.SideIndicator -eq "<="){
                                        ...
                                        if($UpdateableItem.Count -eq 1) { # TEST an EDGE CASE where a GUID appears more than once 
                                            ...
                                            for($UI=0; $UI -lt $UpdateableItem.Count; $UI++){ 
                                                $a = Remove-PnPListItem -Connection $UpdateConnection -List $p.LibPath -Identity $UpdateableItem[$UI].Id -Force -ErrorAction SilentlyContinue -ErrorVariable ErrVar
...
$ping = @{"LastAlive" = get-date ; "Duration(s)" = $JobDuration ; } # add more here for a better log event 
...
#logActivity -Indent 0 -Type "2 Info" -Message "JOB Finished" -logAction "WriteQuiet" # thsi does a log write in each loop so not needed here 
#>